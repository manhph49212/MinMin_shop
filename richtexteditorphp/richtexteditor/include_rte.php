...
class RTEFolderID
{
	...
	function __construct($_x135, $_x63, $category,$_x155)
	{
		if ($_x135 == null)
...
class RTEConfigFile
{
	...
	function ParseCategory($_x46)
	{
		$_x157 = $_x46->getAttribute("for");
		...
		for($_x12=0;$_x12<$_x144->length;$_x12++)
		{
			...
			throw (new Exception("Invalid element '" .$_x139->nodeName."' under '" . $_x46->nodeName . "'"));
		}
	
		
	}
	function ParseSecurity($_x46,$_x158)
	{
		$_x54 = $_x46->getAttribute("name");
		foreach($_x158 as $_x74)
			$_x74->$_x54=$_x46->textContent;
	}
	
	function ParseStorage($_x46, $_x159)
	{
		$_x160 = $_x46->getAttribute("id");
...
class RTEFilter
{
	...
	function DoFixURLType($_x164)
	{
		...
		for ($_x45 = $_x164; $_x45 != null; $_x45 = $_x45->NextNode)
		{
			...
			for($_x171=$_x45->HeadAttribute;$_x171!=null;$_x171= $_x171->NextNode )
			{
				$_x63 = $_x171->Value;

				if (RTEUtil::IsNullOrEmpty($_x63))
					continue;
		...
		for ($_x45 = $_x164; $_x45 != null; $_x45 = $_x45->NextNode)
		{
			...
			for ($_x171 = $_x45->HeadAttribute; $_x171 != null; $_x171 = $_x171 == null ? $_x45->HeadAttribute : $_x171->NextNode)
			{
				...
				if ($this->CheckStyle != null && $_x171->NameLower == "style")
				{
					...
					$_x174 = false;
					for ($_x12 = 0; $_x12 < count($_x114); $_x12++)
					{
						$_x115 = trim($_x114[$_x12]);
						if (strlen($_x115) == 0)
						{
							$_x114[$_x12] = null;
							continue;
	...
	function ParseHtmlCode($_x82)
	{
		...
		while ($_x97<$_x15)
		{
			...
			$_x46=new RTEFilterElement(substr($_x82,$_x97, $_x185 - $_x97));
			if ($_x46->IsValid)
			{
				$_x182=$this->InsertNode($_x46, $_x182);
			}
...
{
	function __construct($_x29, $_x191)
	{
