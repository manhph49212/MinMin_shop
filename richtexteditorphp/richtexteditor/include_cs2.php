...
class CuteSoftAjaxContext
{
	...
	function ConvertFromElement($_x46)
	{
		switch($_x46->getAttribute("t"))
		{
			case "undefined":
				return null;
			case "null":
				return null;
			case "boolean":
				switch(strtolower($_x46->getAttribute("v")))
				{
					...
					case "0":		return false;
					default:
						throw(new Exception("invalid boolean expression:".$_x46->getAttribute("v")));
				}
			case "number":
				return $_x46->getAttribute("v")+0;
			case "string":
				return $_x46->getAttribute("v");
			case "datetime":
				return new CuteSoftDateTime($_x46->getAttribute("v")/1000);
			case "array":
				$_x47=array();
				$_x44=$_x46->childNodes;
				for($_x12=0;$_x12<$_x44->length;$_x12++)
				{
					$_x45=$_x44->item($_x12);
					if($_x45->nodeType!=1)continue;
			...
			case "complex":
				...
				return $_x48;
			default:
				throw(new Exception("unknown @t : ".$_x46->getAttribute("t")));
		}
	}
	function ConvertToElement($item,$_x46)
	{
		if($item===null)
		{
			$_x46->setAttribute("t","null");
			return;
		}
		...
		{
			case "boolean":
				$_x46->setAttribute("t","boolean");
				$_x46->setAttribute("v",$item?"true":"false");
				return;
			case "integer":
			case "double":
				$_x46->setAttribute("t","number");
				$_x46->setAttribute("v",$item);
				return;
			case "string":
				$_x46->setAttribute("t","string");
				$_x46->setAttribute("v",$item);
				return;
			case "array":
				if(count($item)==0)
				{
					$_x46->setAttribute("t","array");
					return;
				}
				if(array_key_exists(0,$item))	
				{
					...
					{
						$_x50=$_x46->ownerDocument->createElement("i");
						$this->ConvertToElement($_x49,$_x50);
						$_x46->appendChild($_x50);
					}
				}
				else
				{
					$_x46->setAttribute("t","complex");
			...
			case "object":
				if($item instanceof CuteSoftDateTime)
				{
					$_x46->setAttribute("t","datetime");
					$_x46->setAttribute("v",$item->GetTime()*1000);
					return;
				}
				$_x46->setAttribute("t","complex");
				foreach($item as $_x51=>$_x49)
