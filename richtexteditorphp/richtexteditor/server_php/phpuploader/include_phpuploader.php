...
class PhpUploader
{
	...
	public $_SourceFileName;
	public $AllowedFileRegExp;
	...
	{
		if(!@$file["tmp_name"])
			throw(new Exception("(1) tmp_name is not available for " . PhpUploader_GetFileName($file) . " , please check PHP post_max_size and upload_max_filesize"));
	...
	{
		$id = @$_GET['_UploadID'];
		if(stripos($id,"<")!==false||stripos($id,">")!==false||stripos($id,"(")!==false||stripos($id,")")!==false||stripos($id,"'")!==false||stripos($id,"\"")!==false||stripos($id,"\\")!==false)
